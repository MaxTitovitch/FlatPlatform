...
{{--            @dump($dialog)--}}
    <div class="pa-dialog-footer">
        ...
        <div class="message-body" data-user-id="{{ Auth::id() }}">
            <div class="display-none super-messager get-action-message" data-message-action="{{ route('get-last-messages', ['id' => 'TOREPLACE']) }}">
                @php
                    $newMessages = null;
                ...
                <details>
                    @foreach($messages as $message)
                        @if(Auth::id() !== $message->user_id)
                            <div class="message-user-all first-user-message my-md-1 text-white"  data-idlast="{{ $message->id }}">
                                <span class="bg-primary px-md-2 rounded">
                                    {{ $message->message }} <span class="message-time">{{ substr($message->created_at, 11, 5) }}</span>
                                </span>
                            </div>
                        @else
                            <div class="message-user-all second-user-message my-md-1 text-white" style="margin-right: 15px;"  data-idlast="{{ $message->id }}">
        ...
        <form id="messageSender" class="footer color-bg-dark-blue w-100 py-md-2 border-bottom border-white" action="{{ route('send-message', ['id' => $dialog->id]) }}" method="post">
            @csrf
            <div class="row bg-white mx-md-5  justify-content-center" style="border-radius: 25px">
                ...
                <div class="">
                    ...
                    <a class="svg-btn" href="#"><svg width="48" height="47" viewBox="0 0 48 47" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g >
                            <path
                ...
                <input type="submit" class=" mr-md-2 my-md-auto border-0 color-bg-dark-blue text-white px-md-2 py-md-1" style="border-radius: 25px">
            </div>
        </form>
    </div>
